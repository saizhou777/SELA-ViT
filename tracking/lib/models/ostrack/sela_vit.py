...
class Attention(nn.Module):
    ...
    def _expand_heads(self, tensor):
        ...
        B, *dim, C =tensor.shape   # (B, N, C) 或 (B, C/h, C)
        ...
        for i in range(1, r_int):
            move_dim =  int(C //self.num_heads * (r_int - i) // r_int)  # 计算要移动的维度数量
        
            rolled_tensor = torch.roll(tensor, shifts=-move_dim, dims=2)   # 使用roll操作进行移动
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        ...
        self.proj = nn.Sequential(
            ...
            #SeparableConv2d(in_chans, embed_dim//4, 3, 2, 1),
            nn.BatchNorm2d(embed_dim//8),
            nn.GELU(),
            nn.Conv2d(embed_dim // 8, embed_dim // 4, 3, 2, 1),
            #SeparableConv2d(embed_dim//2, embed_dim//2, 3, 2, 1),
            nn.BatchNorm2d(embed_dim//4),
            nn.GELU(),
            #SeparableConv2d(embed_dim//2, embed_dim//4*3, 3, 2, 1),
            nn.Conv2d(embed_dim // 4, embed_dim//2, 3, 2, 1),
            nn.BatchNorm2d(embed_dim//2),
            nn.GELU(),
            #SeparableConv2d(embed_dim, embed_dim, 3, 2, 1),
