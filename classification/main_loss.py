...
def main():
    ...
    # linear scale the learning rate according to total batch size, may not be optimal 1024/768 *
    ...
    for epoch in range(config.TRAIN.START_EPOCH, total_epochs):
        ...
        #if dist.get_rank() == 0:
            #logger.info(f"sim_loss_weight: {model.module.sim_loss_weight.item():.4f}")
    ...
    for e in range(50):
        data_loader_train.sampler.set_epoch(e+400)
        update_model(config, data_loader_train, model, logger)

        if dist.get_rank() == 0 and e % config.SAVE_FREQ == 0 :
            save_checkpoint_new(config, e+400, model_without_ddp, max_accuracy, optimizer, lr_scheduler, logger)
...
def update_model(config, data_loader, model, logger):
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        # compute output
        #output = model(images)
        output, attn_similarity_loss = model(images)  # 修改为接收元组输出
        loss = criterion(output, target) + attn_similarity_loss * config.TRAIN.ATTN_SIMILARITY_WEIGHT  # 加入 attn_similarity_loss
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if config.AMP: 
            with autocast():
                ...
                loss = criterion(outputs, targets) + attn_similarity_loss * config.TRAIN.ATTN_SIMILARITY_WEIGHT
        ...
        if (idx + 1) % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                #f'sim_loss_weight {model.module.sim_loss_weight.item():.4f}\t'
                f'mem {memory_used:.0f}MB')
