...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        v = self.v(x)
        expanded_v = self._expand_heads(v)  # B, N, C  ---> B, rh, Ch, N
        lepe = self.lepe(expanded_v.transpose(2, 3).reshape(B, expanded_dim, H, W)).reshape(B, expanded_dim, N).transpose(1, 2)

        attn = (q @ k.transpose(-2, -1)) * self.scale   #B, h, N, N
        expanded_attn = self._expand_heads(attn.permute(0, 2, 1, 3).reshape(B, N, self.num_heads * N ))

        expanded_attn = expanded_attn.softmax(dim=-1)
        expanded_attn = self.attn_drop(expanded_attn)

        out = (expanded_attn @ expanded_v).transpose(1, 2).reshape(B, N, expanded_dim)
    ...
    def _expand_heads(self, tensor):
        ...
        for i in range(1, r_int):
            move_dim = int(dim2 // self.num_heads * (r_int - i) // r_int) 

            rolled_tensor = torch.roll(tensor, shifts=-move_dim, dims=-1) 

            expanded_fea.append(rolled_tensor)

        expanded_fea = torch.cat(expanded_fea, dim=-1)
        expanded_fea = expanded_fea[:, :, :int(dim2 * r)]

        return expanded_fea.reshape(B, *dim1, int(self.num_heads * r), dim2 // self.num_heads).permute(0, 2, 1, 3)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        #x, attn_similarity_loss = self.forward_features(x)
